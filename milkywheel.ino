...
int FilterSteps = 688;                  // Steps required to move the filter to next filter location adjust as required to suit motor wheel combination
...
// Motor definitions
#define STEPS 16                         // 28BYJ-48 steps 4 or 8
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    else if (cmd == "FILTER5") MoveFilter(6); // Calibration Routine
  }
}
