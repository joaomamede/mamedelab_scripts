...
def crunch_data(ficheiro):
    ...
    from support_pla import multiply, make_labels_ws, cell_mask, othercolor, othercolor2, contrast_img
    import pandas as pd
    #for blobs/dots
    plot = False
    minsmask=75
...
#    virus=0
    ...
    reader = pims.bioformats.BioformatsReader(ficheiro,java_memory='12024m')
    ...
    for s in range(nseries):
        ...
        # ~ cells_mask = cell_mask(reader[cells],np.mean(reader[cells]),25000)
    ...
    #        data_virus = othercolor(virus_img,labels_ws)
            ...
            # ~ data_chan3['Filename']s=ficheiro
            data_chan3['Series']=s
...
def main():
    ...
    files_labels = glob.glob(os.getcwd()+'/*.nd2')
    files_labels.sort()
...
# =============================================================================
    ...
    store = pd.HDFStore('stored_data_iGFPlabeel.h5')
    store['df'] = df  # save it
